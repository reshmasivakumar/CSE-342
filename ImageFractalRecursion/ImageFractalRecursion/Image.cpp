...
*/
 ...
 Image Image::mirrorImage() const {
	 ...
	 // a copy of myImage is stored 
	Image mirroredImage = *this;

	for (int row = 0; row < mirroredImage.myImage.rows; row++) {
		for (int col = 0; col < (mirroredImage.myImage.cols / 2); col++) {
			pixel temp = mirroredImage.myImage.pixels[row][col];
			mirroredImage.myImage.pixels[row][col] = mirroredImage.myImage.pixels[row][mirroredImage.myImage.cols - col - 1];
			mirroredImage.myImage.pixels[row][mirroredImage.myImage.cols - col - 1] = temp;
 ...
 */
 ...
 Image Image::photoNegative() const {
	 ...
	 // a copy of myImage is stored 
	Image negativeImage = *this;

	for (int row = 0; row < negativeImage.myImage.rows; row++) {
		for (int col = 0; col < negativeImage.myImage.cols; col++) {
			invertPixels(negativeImage.myImage.pixels[row][col]);
		}
	}
...
void Image::invertPixels(pixel &inputPixel) const {

	inputPixel.blue = 255 - inputPixel.blue;
	inputPixel.red = 255 - inputPixel.red;
	inputPixel.green = 255 - inputPixel.green;
}
